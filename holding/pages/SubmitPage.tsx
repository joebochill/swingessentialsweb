...
import {
  PlaceHolderVideo,
  PlaceHolderText,
} from "../components/display/Placeholder";
...
export const SubmitPage: React.FC = (): JSX.Element => {
  ...
  const loaded = useSelector(
    (state: AppState) => state.api.authentication.initialized
  );
  const getCredits = useSelector(
    (state: AppState) => state.api.getCredits.status
  );
  const getLessons = useSelector(
    (state: AppState) => state.api.loadLessons.status
  );
  const submitStatus = useSelector(
    (state: AppState) => state.api.submitLesson.status
  );
      ...
      <Section>
        <Hidden smDown>
          <div style={{ maxWidth: "40%", alignSelf: "flex-start" }}>
            {/* TODO Use a different component bc we're only doing jumbo */}
            <SectionBlurb
              headline={"Pro Tip #1"}
        ...
        <div className={classes.panel}>
          ...
          {submitStatus === "loading" && (
            <EmptyState
              icon={
                <CloudUpload
                  fontSize={"inherit"} /*htmlColor={Colors.green[500]}*/
                />
              }
