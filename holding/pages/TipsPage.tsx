...
export const TipsPage: React.FC = (): JSX.Element => {
  ...
  const paramIndex =
    id !== undefined ? tips.findIndex((tip) => tip.id === id) : -1;
  ...
  const firstYear =
    tips && tips.length > 0
      ? parseInt(tips[tips.length - 1].date.substr(0, 4), 10)
      : currentYear;
  const lastYear =
    tips && tips.length > 0
      ? parseInt(tips[0].date.substr(0, 4), 10)
      : currentYear;
    ...
    <>
      ...
      {tips.length > 0 && (
        <Section align={"flex-start"}>
              ...
              <CardHeader
                ...
                action={
                  <>
                    <ChevronLeft
                      className={
                        !isLastYear ? classes.chevron : classes.disabled
                      }
                      onClick={
                        !isLastYear
                          ? (): void => setActiveYear(activeYear + 1)
                          : undefined
                      }
                    />
                    <ChevronRight
                      className={
                        !isFirstYear ? classes.chevron : classes.disabled
                      }
                      style={{ marginLeft: 8 }}
                      onClick={
                        !isFirstYear
          ...
          <Spacer classes={{}} flex={0} width={64} />
          {activeTip && (
                  ...
                  <>
                    <IconButton
                      ...
                      onClick={(): void => {
                        ...
                        history.replace(
                          `${ROUTES.TIPS}/${tips[activeIndex - 1].id}`
                        );
                      }}
                    >
