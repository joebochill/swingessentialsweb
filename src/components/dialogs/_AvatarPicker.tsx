...
export const AvatarPickerDialog: React.FC<AvatarDialogProps> = (props) => {
  ...
  const bind = usePinch(
    (state) => {
      ...
      const momentum = state.velocity[0] + state.velocity[1];
...
//   const classes = useStyles();
  ...
  const settingsLoaded = useSelector(
    (state: AppState) => state.api.getUserSettings.status
  );
  ...
  return (
    ...
    >
      ...
      <AvatarPickerDialog
        ...
        src={
          source || `https://www.swingessentials.com/images/profiles/blank.png`
        }
        onClose={(): void => {
